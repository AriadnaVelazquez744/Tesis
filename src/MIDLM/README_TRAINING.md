...
- `src/MIDLM/data/WeaveClinc150_rewritten.json`

## What is implemented (paper mapping)

From `src/MIDLM/Huang_2025-Multi-Intent_Detection_with_Bidirectional_LLM.pdf` §3.2–3.5:
...
```bash
uv run python src/MIDLM/export_textoir_tsv_from_weave_json.py \
  --input-json src/MIDLM/data/WeaveClinc150_rewritten.json \
  --output-dir src/MIDLM/data/textoir_tsv
```

## Models: use your local `src/base_models/`

This repo expects the base models to be stored locally as Hugging Face model folders, e.g.:

- `src/base_models/Qwen2.5-3B-Instruct/`
- `src/base_models/Qwen2.5-7B-Instruct/`
- `src/base_models/Mistral-7B-Instruct-v0.3/`
- `src/base_models/Phi-3-mini-4k-instruct/`
...
- `src/MIDLM/trained_models/<BaseName>_midlm/`
...
```bash
uv run python src/MIDLM/eval_midlm.py \
...
- `src/MIDLM/experiments/<checkpoint>__<split>__<timestamp>/`
  - `metrics.json`
  - `eval_config.json`
...
- `MIDLM.midlm.model.MIDLMForMultiIntent` (intent logits + num logits)
- `MIDLM.midlm.decode.decode_topk_by_predicted_k` (paper-style selection)

LM Studio note:

- LM Studio is useful for *inference* and dataset rewriting, but Unsloth LoRA fine-tuning runs via Python (scripts above).
- `src/base_models/` is used as **read-only input** for training; checkpoints/adapters are always written to `src/MIDLM/trained_models/`.
