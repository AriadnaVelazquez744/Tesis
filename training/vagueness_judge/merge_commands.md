...
- Base models are stored in: `training/base_models/`
...
- **Note:** The training scripts below used original paths (`src/Vagueness_Judge/training/`).  
  After the refactor, these scripts live in `training/vagueness_judge/`.  
  Replace paths as follows before running:
  - `src/Vagueness_Judge/training/sft_3b.py` → `training/vagueness_judge/sft_3b.py`
  - `src/Vagueness_Judge/training/sft_7b.py` → `training/vagueness_judge/sft_7b.py`
  - `src/Vagueness_Judge/training/merge_lora.py` → `training/vagueness_judge/merge_lora.py`
  - `src/Vagueness_Judge/training_models/` → `training/vagueness_judge/adapters/`
  - `src/Vagueness_Judge/trained_models_full/` → `src/Vagueness_Judge/trained_models_full/` (still used at runtime)
...
cd /home/gia/AriadnaVR/JdV_Training && uv run python src/Vagueness_Judge/training/merge_lora.py \
  --base_model_path "training/base_models/Qwen2.5-3B-Instruct" \
...
```bash
cd /home/gia/AriadnaVR/JdV_Training && uv run python src/Vagueness_Judge/training/merge_lora.py \
  --base_model_path "training/base_models/Qwen2.5-7B-Instruct" \
...
cd /home/gia/AriadnaVR/JdV_Training && HF_HOME=.cache uv run python src/Vagueness_Judge/training/merge_lora.py \
  --base_model_path "training/base_models/Phi-3-mini-4k-instruct" \
...
```bash
cd /home/gia/AriadnaVR/JdV_Training && uv run python src/Vagueness_Judge/training/merge_lora.py \
  --base_model_path "training/base_models/Mistral-7B-Instruct-v0.3" \
...
|----------|---------------|-------------|
| `BASE_MODELS_DIR` | `training/base_models/` | Where base models are stored |
