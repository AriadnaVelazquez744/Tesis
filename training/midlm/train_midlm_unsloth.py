...
def load_backbone_with_lora(args: argparse.Namespace):
    base_dir = Path(__file__).resolve().parents[2]  # tesis/
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Subdirectory name inside training/base_models/.",
    )
    p.add_argument(
        "--model_path",
        type=str,
    ...
    p.add_argument(
        "--data_json",
        type=str,
        default=str(SCRIPT_DIR / "data" / "WeaveClinc150_rewritten.json"),
    ...
    # Logging/checkpoints
    p.add_argument("--output_dir", type=str, default=str(SCRIPT_DIR / "adapters" / "trained_models"))
...
def main() -> int:
    ...
    base_models_root = (Path(__file__).resolve().parents[2] / "training" / "base_models").resolve()
